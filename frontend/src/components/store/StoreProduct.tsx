...
export function  StoreProduct({id, name, category, originalPrice, imageUrl}:StoreProducts){
    console.log(id)
...
return (
        <div>
            ...
            <div className={'flex justify-center  pt-0 pb-3 my-2'}>
                ...
                {quantity === 0 ?(
                    <button type="button" className={`${styles.button} bg-fuchsia-400 p-0 hover:bg-red-700 active:bg-blue-400`} onClick={()=>increaseCartQuantity(id)}> + Add To Cart</button>
                ):(
                    <div >
                        <div className={'flex align-middle gap-4 justify-center'}>
                            <button className={'bg-blue-600 px-3 rounded active:bg-red-700'} onClick={()=> decreaseCartQuantity(id)}>-</button>
                            <div >
